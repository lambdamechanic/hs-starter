...
For coverage runs use `cabal test --enable-coverage --enable-per-component --test-show-details=streaming`, ensuring `pg_config --bindir` is on `PATH` so tmp-postgres can locate `initdb`.
